moveUtils = function(){
  ...
  function seekSteer(target,position,velocidadActual, maxForce, maxSpeed){

    var seek = this.seek(target,position).normalize();
    var velDeseada = new Phaser.Point(seek.x*maxSpeed, seek.y*maxSpeed);
    var direccion = MovementUtils.limit(Phaser.Point.subtract(velDeseada,velocidadActual),maxForce)
    return direccion;
  }
  ...
  {  //Funcion que permite que el agente que está en movimiento no se vaya fuera de los límites
    if (vector.x < 0)
    {
